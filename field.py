#!/usr/bin/python

#from field_defs import *
# from color_defs import *
from post import *
from net  import *
from line import *


class Field:
    def __init__(self, sfce, width = FIELD_W, height = FIELD_H):
         self.sfce = sfce
         self.rect = pygame.Rect(0, 0, width, height)
         self.bkgImg = pygame.image.load('pic/bkg11.jpg')
         self.bkgImg = pygame.transform.scale(self.bkgImg, (width, height))
         self.height = height
         self.width  = width
         self.posts = pygame.sprite.Group([])
         self.FieldSfce = self.bkgImg
         self.draw()
         self.sfce.blit(self.FieldSfce, (0,0))

    def draw(self):
        Line(BORDER_TOP_IMG, (0, 0), (self.width, BORDER_HEIGHT), self.FieldSfce)
        Line(BORDER_BOT_IMG, (0, self.height - BORDER_HEIGHT), (self.width, BORDER_HEIGHT), self.FieldSfce)
        Line(CENTER_LINE_IMG, self.rect.center, (0, 0), self.FieldSfce)
#       pygame.draw.line(self.FieldSfce, SNOW, (self.FieldRect.topright), (self.FieldRect.bottomright), 1)
#       pygame.draw.line(self.FieldSfce, SNOW, (self.FieldRect.midtop), (self.FieldRect.midbottom), 3)
#       pygame.draw.circle(self.FieldSfce, SNOW, (self.FieldRect.centerx+1, self.FieldRect.centery+1), FIELD_CENTER_SIZE, 3)
        Line(GOAL_LINE_IMG, (self.rect.left+NET_SIZE, self.rect.centery), (0, 0), self.FieldSfce)
        Line(GOAL_LINE_IMG, (self.rect.right-NET_SIZE, self.rect.centery), (0, 0), self.FieldSfce)
        Net(self.FieldSfce, 0, 0, NET_SIZE, FIELD_H)
        Net(self.FieldSfce, FIELD_W - NET_SIZE+1, 0, NET_SIZE*2, FIELD_H)
        Post(POST_IMG, (POST_SIZE, ), (POST_RIGHT_X, POST_TOP_Y), self.posts, self.FieldSfce)
        Post(POST_IMG, (POST_SIZE, ), (POST_LEFT_X, POST_TOP_Y), self.posts, self.FieldSfce)
        Post(POST_IMG, (POST_SIZE, ), (POST_LEFT_X, POST_BOT_Y), self.posts, self.FieldSfce)
        Post(POST_IMG, (POST_SIZE, ), (POST_RIGHT_X, POST_BOT_Y), self.posts, self.FieldSfce)



