...
class Field:
    def __init__(self, sfce, width=FIELD_W, height=FIELD_H):
        ...
        self.FieldSfce = self.bkgImg
        self.draw()
        self.sfce.blit(self.FieldSfce, (0, 0))

    def draw(self):
        Line(BORDER_TOP_IMG, (0, 0), (self.width, BORDER_HEIGHT), sfce=self.FieldSfce)
        Line(BORDER_BOT_IMG, (0, self.height - BORDER_HEIGHT), (self.width, BORDER_HEIGHT), sfce=self.FieldSfce)
        Line(CENTER_LINE_IMG, (self.rect.centerx, self.rect.centery), (0, 0),  sfce=self.FieldSfce)
...
#       pygame.draw.circle(self.FieldSfce, SNOW, (self.FieldRect.centerx+1, self.FieldRect.centery+1), FIELD_CENTER_SIZE, 3)
        ...
        Net(self.FieldSfce, FIELD_W - NET_SIZE+1, 0, NET_SIZE*2, FIELD_H)
        Line(BORDER_SIDE_IMG, (self.width-1, self.rect.centery), (0, 0), sfce=self.sfce)
        line_goalleft=Line(GOAL_LINE_IMG, (POST_LEFT_X, (POST_TOP_Y+POST_BOT_Y)/2), (0, 0), self.goals, self.FieldSfce)
        ...
        self.FieldSfce.blit(line_topleft.image, line_topleft.rect)
        line_botleft=Line(BORDER_SIDE_IMG, (POST_LEFT_X, 0), (0, 0))
        line_botleft.rect.midtop = (POST_LEFT_X, POST_BOT_Y)
        self.FieldSfce.blit(line_botleft.image, line_botleft.rect)

        line_goalright=Line(GOAL_LINE_IMG, (POST_RIGHT_X, (POST_TOP_Y+POST_BOT_Y)/2), (0, 0), self.goals, self.FieldSfce)
        ...
        self.FieldSfce.blit(line_topright.image, line_topright.rect)
        line_botright=Line(BORDER_SIDE_IMG, (POST_RIGHT_X, 0), (0, 0))
        line_botright.rect.midtop = (POST_RIGHT_X, POST_BOT_Y)
        self.FieldSfce.blit(line_botright.image, line_botright.rect)
